/*
 * DAA2RMA.java Copyright (C) 2024 Daniel H. Huson
 *
 ...
 * Daniel Huson, 8.2015
 */
public class DAA2RMA {
	/**
	 * merge RMA files
	 */
	public static void main(String[] args) {
		try {
			ProgramProperties.setProgramName("add2rma");
			Setup.apply();

			PeakMemoryUsageMonitor.start();
			(new DAA2RMA()).run(args);
	...
	private void run(String[] args) throws UsageException, IOException, CanceledException, SQLException {
		...
		options.setLatexDescription("""
				...
				based on the alignments, and writes out the reads and alignments, together with the classifications
				and indices to file in RMA (read-match archive) format that can then be opened in MEGAN.
				Do not use this legacy program, rather use the \\verb^daa-meganizer^ program instead.
				""");
		...
		for (int i = 0; i < daaFiles.length; i++) {
			...
			if (!processInPairs)
				createRMA6FileFromDAA("DAA2RMA", daaFiles[i], outputFiles[iOutput], useCompression, doc, maxMatchesPerRead, progressListener);
			else
				createRMA6FileFromDAAPair("DAA2RMA", daaFiles[i], daaFiles[i + 1], outputFiles[iOutput], useCompression, doc, maxMatchesPerRead, progressListener);
