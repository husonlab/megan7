...
public class ClassificationRegistration {
	...
	public static void register(boolean verbose) {
		try {
			final Set<String> treeFiles = Arrays.stream(ResourceUtils.fetchResources(Megan7.class, "megan/resources/files")).filter(s -> s.endsWith(".tre")).collect(Collectors.toSet());
			final Set<String> mapFiles = Arrays.stream(ResourceUtils.fetchResources(Megan7.class, "megan/resources/files")).filter(s -> s.endsWith(".map")).collect(Collectors.toSet());
