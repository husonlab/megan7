/*
 * Sam2Rma.java Copyright (C) 2024 Daniel H. Huson
 *
 ...
 * Daniel Huson, 3.2012
 */
public class Sam2Rma {
	/**
	 * merge RMA files
	 */
	public static void main(String[] args) {
		try {
			ProgramProperties.setProgramName("sam2rma");
			Setup.apply();

			PeakMemoryUsageMonitor.start();
			(new Sam2Rma()).run(args);
	...
	private void run(String[] args) throws UsageException, IOException, SQLException {
		...
		options.setLatexDescription("""
				This takes an alignment file in SAM format, produced by MALT,
				as input (can also run on multiple input files), classifies the taxonomic
		...
		final String[] samFiles = options.getOptionMandatory("-i", "in", "Input SAM file[s] generated MALT (gzipped ok)", new String[0]);
		...
		for (int i = 0; i < samFiles.length; i++) {
			...
			createRMA6FileFromSAM("Sam2Rma", samFiles[i], readsFiles[i], outputFiles[i], useCompression, doc, maxMatchesPerRead, progressListener);
