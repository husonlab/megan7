/*
 * Blast2Rma.java Copyright (C) 2024 Daniel H. Huson
 *
...
/**
 * compute an RMA6 file from a Blast file
 * Daniel Huson, 3.2012
 */
public class Blast2Rma {
	/**
	 * merge RMA files
	 */
	...
	private void run(String[] args) throws UsageException, IOException, SQLException {
		...
		options.setLatexDescription("""
				...
				and functional content and then writes out the reads and alignments, together with the classifications
				and indices to file in RMA (read-match archive) format that can then be opened in MEGAN.
				For DAA files produced by DIAMOND, use the \\verb^daa-meganizer^ program instead.
				""");
		...
		for (int i = 0; i < blastFiles.length; i++) {
			...
			if (!processInPairs)
				createRMA6FileFromBLAST("Blast2Rma", blastFiles[i], blastFormat, readsFiles[i], outputFiles[iOutput], useCompression, doc, maxMatchesPerRead, progressListener);
			else
				createRMA6FileFromBLASTPair("Blast2Rma", blastFiles[i], blastFiles[i + 1], blastFormat, readsFiles[i], readsFiles[i + 1], outputFiles[iOutput], useCompression, doc, maxMatchesPerRead, progressListener);
